...
// It's best practice to rely on the environment variable set in Vercel and Api.js
// const API_URL = 'https://hacktrail-team-orion-x-git-host-ghdbashens-projects.vercel.app/api';
...
        const response = await api.get('/foods/availability');
        // FIX: Axios responses are already JSON, access data with response.data
...
                          src={`${process.env.REACT_APP_API_URL.replace('/api', '')}/${item.image_path.replace(/\\/g, '/')}`} 
                          alt={item.food_name} 
