...
export const getCustomerByEmailORPswdModel = async (credential) => {
  const [result] = await pool.query(
    'SELECT cus_id, password, email, role, phone, refresh_token FROM customers WHERE email = ? OR phone = ?',
...
export const addCustomerModel = async (name, email, phone, password) => {
  console.log("Adding customer:", { name, email, phone, password });
  ...
  const [result] = await pool.query(
    'INSERT INTO customers (name, phone, email, role, created_at, password) VALUES (?, ?, ?, ?, ?, ?)',
    [name, phone, email, 'customer', created_at, hashedPassword]
  );
  // Fetch the generated customer_id (since it's generated by trigger)
  const [rows] = await pool.query(
    'SELECT * FROM customers WHERE email = ? AND phone = ? ORDER BY created_at DESC LIMIT 1',
    [email, phone]
  );
  return rows[0];
}
